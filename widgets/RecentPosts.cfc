	...
	any function renderIt(
		...
		saveContent variable="rString"{
			...
			for(var x=1; x lte arguments.max; x++){
				writeOutput('<div class="last-post media">');
				if(entryResults.content[ x ].getfeaturedImageURL() NEQ ""){
					writeOutput('<a href="#cb.linkEntry(entryResults.content[ x ])#" class="blog-imgOuter"> <img class="img-fluid" src="#entryResults.content[ x ].getFeaturedImageURL()#" alt="#entryResults.content[ x ].getTitle()#"></a>');
				}
				writeOutput('<div class="content media-body">');
				writeOutput('<span>#dateFormat(entryResults.content[ x ].getDisplayPublishedDate(),'dd mmmm yyyy' )#</span>');
