		...
		saveContent variable="rString"{
			...
			for(var x=1; x lte arguments.max; x++){
				...
				if(entryResults.entries[ x ].getfeaturedImageURL() NEQ ""){
					writeOutput('<a href="#cb.linkEntry(entryResults.entries[ x ])#" class="blog-imgOuter"> <img class="img-fluid" src="#entryResults.entries[ x ].getFeaturedImageURL()#" alt="#entryResults.entries[ x ].getTitle()#"></a>');
				}
				writeOutput('<div class="content media-body">');
				writeOutput('<span>#dateFormat(entryResults.entries[ x ].getDisplayPublishedDate(),'dd mmmm yyyy' )#</span>');
